...
**Last Updated:** 2026-08-26T08:04:00.000Z
**Active Feature:** Elasticsearch 数据导出与导入
**Current RUP Phase:** construction
**Current Iteration:** 核心功能开发迭代（Elasticsearch 迁移）
...
1. 评估者按迭代协议验收 `elasticsearch-migration`。
2. 读取 `feature_list.json`，选择 `large-data-migration` 作为下一个 construction feature。
3. 编写大数据量任务与可靠性迭代协议，并按协议实现、测试、评估、复盘。
...
先运行 `bash init.sh` 确认基线健康，再从 `feature_list.json` 选择唯一一个未完成 feature。Docker Elasticsearch 集成测试可用 `ELASTICSEARCH_INTEGRATION=1 ELASTICSEARCH_INTEGRATION_PORT=9201 npx vitest run tests/elasticsearch.integration.test.ts` 复跑；PostgreSQL 集成测试命令保持不变。
